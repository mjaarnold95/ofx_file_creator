...
def main(checking_path: Path, savings_path: Path):
    # Process checking account
    if checking_path.exists():
        try:
            df = load_and_prepare(checking_path)
    ...
    if savings_path.exists():
        try:
            df = load_and_prepare(savings_path)
...
if __name__ == "__main__":
    ...
    main(checking_path=checking_path, savings_path=savings_path)
